...
public class MedicionRepositoryImpl implements MedicionRepositoryPort {
    ...
    public List<Medicion> buscarPorRecurso(Long idRecurso) {
        return jpa.findByIdRecurso(idRecurso).stream().map(this::toDomain).collect(Collectors.toList());
    }
    ...
    private MedicionEntity toEntity(Medicion m) {
        return new MedicionEntity(m.getId(), m.getPh(), m.getTemperatura(), m.getFecha(), m.getIdUsuario(), m.getIdRecurso(), m.getIdContaminante());
    }

    private Medicion toDomain(MedicionEntity e) {
        return new Medicion(e.getId(), e.getPh(), e.getTemperatura(), e.getFecha(), e.getIdUsuario(), e.getIdRecurso(), e.getIdContaminante());
    }
}